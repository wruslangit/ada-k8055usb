...
#include <assert.h>
#include <math.h>
// #include "k8055.h"   // MODIFIED BY WRY

#include "ck8055.h"     // MODIFIED BY WRY
#include <sys/types.h>  // ADDED BY WRY

#define VERSION "Version 1.0 WRY"
...
int SetCounterDebounceTime(long CounterNo, long DebounceTime)
{
    ...
    if (CounterNo == 1 || CounterNo == 2)
    {
        ...
        if (DebounceTime > 7450)
            DebounceTime = 7450;
        value = (float)sqrtf(DebounceTime / 0.115);
